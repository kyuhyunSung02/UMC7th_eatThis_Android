# 이거먹자-Android
> UMC 7th 1st Place Team <br>
24.12.30 - 25.02.21

<img width="7201" height="1201" alt="image" src="https://github.com/user-attachments/assets/e02da149-30b3-4ade-8f67-329e40bf4595" />

<div align="center">
 
###  **프로젝트 전체 보기**
[![UMC 7th - 이거먹자](https://img.shields.io/badge/UMC_7th-이거먹자-FF6F00?style=for-the-badge&logoColor=white)](https://github.com/UMC-7th)


</div>


![image](https://github.com/user-attachments/assets/9d56917b-472a-4e8b-8dec-2a82962368e5)

## 📝 *****Information Architecture*****

<img width="1920" height="1081" alt="image" src="https://github.com/user-attachments/assets/02977274-2492-4366-9d4f-06c76656935f" />


## 🍨 *****Contributors*****

| 손주완(Lead) <br> [@vvan2](https://github.com/vvan2) | 김민주 <br> [@MINJU](https://github.com/MINJU-62) | 성규현 <br> [@kyuhyunSung02](https://github.com/kyuhyunSung02) <br> [@dmp100](https://github.com/dmp100)| 최희원 <br> [@Choiheewon](https://github.com/heewon411) |
|:---:|:---:|:---:|:---:|
| <img width="200" src="https://github.com/user-attachments/assets/40d71434-fafc-437a-a8c9-2dcc276bc329"/> | <img width="200" src="https://github.com/user-attachments/assets/2204e157-2142-41a1-9357-5166daabb644"/> | <img width="200" src="https://github.com/user-attachments/assets/48571048-0733-41db-923f-83b97fa858ea"/> | <img width="200" src="https://github.com/user-attachments/assets/8631248c-6d9b-49f4-95f2-f956d3c22393"/> |
|`스플래시`   `로그인`  `회원가입`<br> `애니메이션`  `알림설정` <br>   `내 정보관리` `장바구니` <br> `결제` `시세/상세페이지` | `식단/오늘`  `식단 등록내역` <br> `수동등록` `식재료 시세`<br> `식단 레시피`  `맛있는 일상 구독` <br> `즐겨찾기`  | `카카오로그인` `식단/월간`  `시세/메인` <br> `구독관리`  `구독내역` <br> `구독식단조회`    | `온보딩` `설문조사` <br> `주소록 관리`|



<br/>

## 🟨 *****SCREENSHOT*****

| 온보딩,로그인 | 설문조사 | 식단 |
|:---:|:---:|:---:|
| <img width="200" src="https://github.com/user-attachments/assets/c43c2fec-6e2a-459f-9b71-3637db952e83"/> | <img width="200" src="https://github.com/user-attachments/assets/e3731a75-dee7-435b-8f86-b7325acaa921"/> | <img width="200" src="https://github.com/user-attachments/assets/ac7c9ee3-0a1b-415f-be58-14e33da43123"/> |

| 시세 | 구독 | 마이 |
|:---:|:---:|:---:|
| <img width="200" src="https://github.com/user-attachments/assets/bd749f82-3427-4b4e-83f1-18e468ed5508"/> | <img width="200" src="https://github.com/user-attachments/assets/d94119f8-afb6-4ecd-b30e-d5286c632cd1"/> | <img width="200" src="https://github.com/user-attachments/assets/fd0df426-9ee2-4cc6-b04d-57dd5ab6c5e7"/> |


---

## 🌟 **주요 기능 (Android App)**

### 🥗 **식단 제공 서비스**

* **맞춤형 식단 추천**: AI 영양사가 개인 건강/목표에 맞는 식단 제공
* **다양한 단위**: 오늘 / 주간 / 월간 단위로 관리 가능
* **체계적 관리**: 식습관 분석 기반 균형 잡힌 식단 제안

### 💰 **시세 서비스**

* **실시간 가격 확인**: 식재료 시세 실시간 제공
* **구매 연동**: 마켓/쿠팡 등 쇼핑 플랫폼 연동
* **시세 차트**: 가격 변동 추세 확인 가능
* **합리적 소비 지원**: 경제적인 식단 구성 가능

### 📦 **구독 서비스**

* **정기 구독**: 주/월 단위 맞춤형 식단 제공
* **배송 선택**: 다양한 배송 옵션으로 편리한 이용 가능
* **지속 관리**: 장기적인 건강 관리 가능

### 🧬 **나의 건강 서비스**

* **건강 분석**: 영양 균형, 체질, 생활 습관 분석
* **변화 추적**: 체중, 신체 변화, 영양 상태 기록
* **AI 개선 플랜**: 개인 맞춤 건강 관리 플랜 제공

---

## 💡 **프로젝트 배경**

**이거먹자**는 **AI 기반 개인 맞춤 식단 관리 서비스**로,
“오늘 뭐 먹지?”라는 일상적인 고민부터 장기적인 건강 관리까지 해결하는 **AI 영양사 플랫폼**입니다.

### **해결하고자 하는 문제**

* 🍽️ **매일 반복되는 식사 고민**: “오늘 뭐 먹지?”를 AI가 해결
* 🥦 **건강 관리의 어려움**: 식단 균형·영양 분석을 전문가처럼 제공
* 📉 **경제적 부담**: 실시간 시세 반영으로 합리적 소비 지원
* 📊 **장기 관리 부족**: 구독 서비스와 건강 기록으로 꾸준한 관리 가능

### **핵심 가치**

* **🤖 AI 기반**: 개인 맞춤 식단과 건강 분석 제공
* **🥗 건강 중심**: 균형 잡힌 식단으로 생활습관 개선
* **💰 합리적 소비**: 시세 정보를 통한 경제적 선택
* **📈 지속 가능성**: 구독 서비스와 건강 추적을 통한 장기 관리

---

## 📊 **기대 효과**

1. **⏰ 시간 절약**: 식단 고민 없이 AI가 자동 추천
2. **🥦 건강 증진**: 맞춤 영양 관리로 생활습관 개선
3. **💰 비용 절감**: 실시간 시세 반영으로 경제적 식단 구성
4. **📈 장기적 효과**: 구독 + 건강 기록으로 꾸준한 관리 가능

---


## 📗 *****Convention*****
[📱 FE 공간](https://www.notion.so/FE-1867ed2cd0e181598911e1e71e3aa139?pvs=4)
<br>
[🚀 Git Convention + Branch Strategy](https://www.notion.so/Git-Convention-Branch-Strategy-1867ed2cd0e18188a6eff164b18c7965?pvs=4)
<br>
[✏️Figma](https://www.figma.com/design/XswgLf9BzXFH49Ae4N578k/Wireframe_Kim-Taehyun(Tommy)?node-id=279-1632&p=f&t=YE8OZFATJhky8Zfc-0)
<br/>

## 🔧 *****TECH STACKS*****
| **Category** | **TechStack** |
| --- | --- |
| Language | Kotlin |
| Network | Retrofit, OkHttp, Gson |
| Asynchronous | Coroutines |
| Jetpack | ViewBinding, Navigation, DataStore |
| Image | Glide |

<br/>

## 🍚 **이거먹자 시연 영상**
> ### 🔗 아래 이미지를 클릭하면 YouTube 시연 영상으로 이동합니다

[![이거먹자 Demo ](https://github.com/user-attachments/assets/c656bd12-9562-4cf6-a728-f43c8d1cc559)](https://youtu.be/rcHvzdXhWX0)


## 📁 *****Foldering*****

```
📂 com.example.umc
┣ 📂 core
┃ ┣ 📂 designsystem
┃ ┃ ┣ 📂 theme
┃ ┃ ┣ 📂 component
┃ ┃ ┗ 📂 util
┃ ┣ 📂 navigation
┃ ┗ 📂 state
┣ 📂 data
┃ ┣ 📂 datasource
┃ ┣ 📂 datasourceimpl  
┃ ┣ 📂 model
┃ ┃ ┣ 📂 request
┃ ┃ ┗ 📂 response
┃ ┣ 📂 repositoryimpl
┃ ┗ 📂 service
┣ 📂 domain
┃ ┣ 📂 model
┃ ┣ 📂 repository
┃ ┗ 📂 usecase
┃   ┣ 📂 user
┃   ┗ 📂 diet
┗ 📂 presentation
 ┣ 📂 diet
 ┃ ┗ 📂 adapter
 ┣ 📂 mypage
 ┣ 📂 survey  
 ┗ 📂 subscribe
